...
class BaseNet(nn.Module):
    def __init__(self, nclass, backbone, aux, se_loss, jpu=True, dilated=False, norm_layer=None,
                 ...
                 std=[.229, .224, .225], root='~/.encoding/models',in_channels=102, **kwargs):
        ...
        # # 如果输入通道数不是3，则调整权重
        # if self.conv1[0].in_channels != 3:
        #     pretrained_weight = self.conv1[0].weight.data
        #     # 计算平均值
        #     avg_weight = pretrained_weight.mean(dim=1, keepdim=True)
        #     # 扩展到新的通道数
        #     new_weight = avg_weight.expand(pretrained_weight.shape[0], self.conv1[0].in_channels, 
        #                                    pretrained_weight.shape[2], pretrained_weight.shape[3])
        #     self.conv1[0].weight.data = new_weight

            self.pretrained = resnet.resnet50(pretrained=True, dilated=dilated,
                                              norm_layer=norm_layer, root=root,in_channels=in_channels)
            
            if in_channels != 3:
                # 获取原始卷积层
                if isinstance(self.pretrained.conv1, nn.Sequential):
                    conv1_layer = self.pretrained.conv1[0]
                else:
                    conv1_layer = self.pretrained.conv1
                
                # 保存原始权重
                old_weight = conv1_layer.weight.data
                
                # 创建新的卷积层
                new_conv = nn.Conv2d(
                    in_channels, 
                    conv1_layer.out_channels,
                    kernel_size=conv1_layer.kernel_size,
                    stride=conv1_layer.stride,
                    padding=conv1_layer.padding,
                    bias=conv1_layer.bias is not None
                )
                ...
                with torch.no_grad():
                    ...
                    # 复制偏置项
                    if conv1_layer.bias is not None:
                        new_conv.bias.data = conv1_layer.bias.data
                
                # 替换卷积层
                if isinstance(self.pretrained.conv1, nn.Sequential):
                    self.pretrained.conv1[0] = new_conv
                else:
                    self.pretrained.conv1 = new_conv
...
class MultiEvalModule(DataParallel):
    ...
    def parallel_forward(self, ir_inputs, vis_inputs, **kwargs):
        ...
        inputs = [(ir_input.unsqueeze(0).cuda(device), vis_input.unsqueeze(0).cuda(device))
              for ir_input, vis_input, device in zip(ir_inputs, vis_inputs, self.device_ids)]
