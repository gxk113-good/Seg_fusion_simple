...
# fcn.py 修改后的 get_fcn 函数
def get_fcn(dataset='pascal_voc', backbone='resnet50', pretrained=False,
            root='~/.encoding/models', nclass=None, **kwargs):  # 添加 nclass 参数
    # if nclass is None:
    #     # 仅当未手动指定 nclass 时，才从预定义数据集获取
    #     from ..datasets import datasets
    #     nclass = datasets[dataset.lower()].NUM_CLASS
    # # 其他代码保持不变...
    model = FCN(nclass, backbone=backbone, root=root, **kwargs)
