...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, dilated=True,
                 deep_base=True, norm_layer=nn.BatchNorm2d, output_size=8,in_channels=3):
...
# class ResNet(nn.Module):
#     def __init__(self, block, layers, in_channels=3, num_classes=1000, dilated=False, 
#                  deep_base=True, norm_layer=None):
#         super(ResNet, self).__init__()
#         # ... 原有代码 ...
...
def resnet50(pretrained=False, root='~/.encoding/models', in_channels=3, **kwargs):
    """Constructs a ResNet-50 model with adaptable input channels."""
    model = ResNet(Bottleneck, [3, 4, 6, 3], in_channels=in_channels, **kwargs)
    
    if pretrained:
        from ..models.model_store import get_model_file
        state_dict = torch.load(get_model_file('resnet50', root=root))
        
        # 适配第一层卷积权重
        conv1_key = 'conv1.0.weight' if model.deep_base else 'conv1.weight'
        if conv1_key in state_dict:
            old_weight = state_dict[conv1_key]
            ...
            if in_channels != 3:
                ...
                state_dict[conv1_key] = new_weight
            else:
                # 保持原始权重不变
                pass
