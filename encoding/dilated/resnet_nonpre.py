    ...
    def __init__(self, block, layers, num_classes=1000, dilated=True,
        ...
        if deep_base:
            self.conv1 = nn.Sequential(
                nn.Conv2d(102, 64, kernel_size=3, stride=2, padding=1, bias=False),
                norm_layer(64),
            ...
            self.conv1 = nn.Conv2d(102, 64, kernel_size=7, stride=2, padding=3,
                                   bias=False)
...
#             if in_channels > 3:
#                 # 多通道情况下复制均值
#                 mean_weight = conv1_weight.mean(dim=1, keepdim=True)
#                 new_weight = mean_weight.repeat(1, in_channels//3, 1, 1)
#             else:
#                 # 单通道情况下取均值
#                 new_weight = conv1_weight.mean(dim=1, keepdim=True)
