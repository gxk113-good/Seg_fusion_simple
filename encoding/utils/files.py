...
def download(url, path=None, overwrite=False, sha1_hash=None):
    ...
    expected_filename = "resnet50-ebb6acbb.pth"  # 与你的文件名一致
    local_path = os.path.join(os.path.expanduser('~/.encoding/models/'), expected_filename)
        
    if os.path.exists(local_path):
            print(f"使用本地预训练模型: {local_path}")
            return local_path  # 直接返回本地路径
